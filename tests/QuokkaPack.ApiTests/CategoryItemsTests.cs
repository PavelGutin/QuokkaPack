...
namespace QuokkaPack.ApiTests.Controllers
{
    public class CategoryItemsTests : IClassFixture<ApiTestFactory>
    {
        ...
        {
            var response = await _client.GetAsync("/api/categoryitems");
        ...
        {
            var response = await _client.GetAsync("/api/categoryitems/9999");
        ...
        public async Task Delete_ShouldReturnNoContent_WhenValid()
        {
            ...
            var deleteResponse = await _client.DeleteAsync($"/api/categoryitems/{id}");
